<div>
	<h2>产品类别</h2>
    <p class="page_info">对产品类别进行管理，编辑等</p>
</div>
<table class="bk_form_tbl">
    <tr>
        ...
        <?php
            $str = '';
            function createstype($data, &$str,&$total,$id,$pid)
            {
                foreach ($data as $k=>$v) {
                    if(!empty($v)) {
            ...
            foreach ($ptype as $row){
                ...
                if (!empty($row['son']))
                    createstype($row['son'],$str, $total,$info['id'],$info['pid']);
            } 
        ?>
...
function savetype()
{
	...
	if (pid==typeid) pid=0;
	var postdata = {'id': typeid, 'pid': pid, 'type': 'mtype','typename': typename}
