...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    //---colore and sizes
    $goods_attr = get_attr_list($goods['goods_type'],$goods['goods_id']);
    ...
    //--- extends attribute
    $goods_extends = get_goods_extends_attr($goods['goods_id']);
    $goods_extends_info = array();
    if ($goods_extends) {
        foreach ($goods_extends as $k=>$v) {
            $goods_extends_info[$k]['info'] = $v;
            $goods_extends_info[$k]['num'] = count($v);
        }
    }
    print_r($goods_extends_info);
    $smarty->assign('goods_extends_info', $goods_extends_info);
    //--
    $volume_price_list = '';
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    /* 处理相册图片 */
    handle_gallery_image($goods_id, $_FILES['img_url'], $_POST['img_desc'], $_POST['img_file'], $_POST['img_color']);
...
elseif ($_POST['act'] == 'addGoodsAttrExt'){
    
    $gid = (int) $_POST['gid'];
    $color = $_POST['color'];
    $size = $_POST['size'];
    
    $sql = "INSERT INTO ecs_goods_attr_ext(gid, color, size) VALUES ($gid, '$color', '$size')";
    if ($GLOBALS['db']->query($sql))
        echo $GLOBALS['db']->insert_id();
}
elseif ($_POST['act'] == 'delGoodsAttrExt') {
    $gid = (int) $_POST['gid'];
    $color = trim($_POST['color']);
    $size = trim($_POST['size']);

    if ($color != '' && $size == '')
        $sql = "DELETE FROM ecs_goods_attr_ext WHERE gid=$gid AND color='$color'";
    elseif ($size != '' && $color =='')
        $sql = "DELETE FROM ecs_goods_attr_ext WHERE gid=$gid AND size='$size'";
...
//get goods extends attribute
function get_goods_extends_attr($goods_id)
{
    if (!$goods_id) return false;
    
    $sql = "SELECT * FROM ecs_goods_attr_ext WHERE gid=$goods_id  order by color";
...
//get goods extends color
function get_goods_extends_attr_color($goods_id)
{
    if (!$goods_id) return false;
    
    $sql = "SELECT distinct color from ecs_goods_attr_ext WHERE gid=$goods_id";
    return $GLOBAL['db']->getAll($sql);
}
//get goods extend size
function get_goods_extends_attr_size($goods_id)
{
    if (!$goods_id) return false;

    $sql = "SELECT distinct size from ecs_goods_attr_ext WHERE gid=$goods_id";
    return $GLOBAL['db']->getAll($sql);
}
function get_goods_extends_attr_html($goods_type,$goods_id)
{
    $goods_attr = get_attr_list($goods_type,$goods_id);
    foreach ($goods_attr as $k=>$v) {
        if('颜色' == $v['attr_name'])
            $colors = explode("\n",$v['attr_values']);
        if('尺寸' == $v['attr_name'])
    ...
    $str = '';
    if ($colors) {
        $str = "<tr style='display:none;'><td><span id='goodsid'>{$goods_id}</span></td></tr>".
                ""
    }
}
?>
