 ...
 * @return  array
 */
function get_goods_gallery($goods_id, $img_color='')
{
    if ('' == $img_color)
        $where_color = "1";
    else
        $where_color = "color='$img_color'";
    $sql = 'SELECT img_id, img_url, thumb_url, img_desc,img_color' .
        ' FROM ' . $GLOBALS['ecs']->table('goods_gallery') .
        " WHERE goods_id = '$goods_id' AND $where_color ORDER BY img_color LIMIT " . $GLOBALS['_CFG']['goods_gallery_number'];
...
function get_goods_gallery_color($goods_id)
{
    $sql = "SELECT DISTINCT img_color FROM ecs_goods_gallery WHERE goods_id=$goods_id";
    $res = $GLOBALS['db']->getAll($sql);
    if(1 == count($res) && $res[0]['img_color']==0)
        return false;
    else
        return $res;
}
