...
<!--            <td><input type="text" name="goods_number" value="<?php echo $this->_var['goods']['goods_number']; ?>" size="20" <?php if ($this->_var['code'] != '' || $this->_var['goods']['_attribute'] != ''): ?>readonly="readonly"<?php endif; ?> /><br />-->
        ...
        -->
<?php
echo file_get_contents('/var/www/52sports/admin/templates/goods_extends_attr.php');
?>
        <?php if ($this->_var['colors']): ?>
        <tr style="display:none;"><td><span id="goodsid"><?php echo $this->_var['goods']['goods_id']; ?></span></td></tr>
        <tr><td>
        <table id="goods_attr_ext-tables"  align="center">
            <tr>
                <td>颜色：</td>
                <td>
                    <?php $_from = $this->_var['colors']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('k', 'v');if (count($_from)):
    foreach ($_from AS $this->_var['k'] => $this->_var['v']):
?>
                        <input type="checkbox" value="<?php echo $this->_var['v']; ?>" class="goods_attr_color" extid="<?php echo $this->_var['k']; ?>" /><?php echo $this->_var['v']; ?>
                    <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
                </td>
            </tr>
            <tr>
                <td>尺码：</td>
                <td>
                    <?php $_from = $this->_var['sizes']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('k', 'v');if (count($_from)):
    foreach ($_from AS $this->_var['k'] => $this->_var['v']):
?>
                        <input type="checkbox" value="<?php echo $this->_var['v']; ?>" class="goods_attr_size" extid="<?php echo $this->_var['k']; ?>" /><?php echo $this->_var['v']; ?>
                    <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    <table id="goods_attr_ext_table">
                        ...
                        <?php if ($this->_var['goods_extends_info']): ?>
                            <?php $_from = $this->_var['goods_extends_info']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'attrGroup');if (count($_from)):
    foreach ($_from AS $this->_var['attrGroup']):
?>
                                <?php $_from = $this->_var['attrGroup']['info']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('gk', 'attrInfo');if (count($_from)):
    foreach ($_from AS $this->_var['gk'] => $this->_var['attrInfo']):
?>
                                    <?php if ($this->_var['gk'] == 0): ?>
                                        <tr>
                                        <td rowspan="<?php echo $this->_var['attrGroup']['num']; ?>"><?php echo $this->_var['attrInfo']['color']; ?></td>
                                        <td><?php echo $this->_var['attrInfo']['size']; ?></td>
                                        <td><input type="text" goodsattrid="<?php echo $this->_var['attrInfo']['id']; ?>" value="<?php echo $this->_var['attrInfo']['price']; ?>" /></td>
                                        <td><input type="text" goodsattrid="<?php echo $this->_var['attrInfo']['id']; ?>" value="<?php echo $this->_var['attrInfo']['num']; ?>" /></td>
                                        <td><input type="text" goodsattrid="<?php echo $this->_var['attrInfo']['id']; ?>" value="<?php echo $this->_var['attrInfo']['code']; ?>" /></td></tr>
                                    <?php else: ?>
                                        <tr>
                                        <td><?php echo $this->_var['attrInfo']['size']; ?></td>
                                        <td><input type="text" goodsattrid="<?php echo $this->_var['attrInfo']['id']; ?>" value="<?php echo $this->_var['attrInfo']['price']; ?>" /></td>
...
?>
              <div id="gallery_<?php echo $this->_var['img']['img_id']; ?>" style="float:left; text-align:center; border: 1px solid #DADADA; margin: 4px; padding:2px;">
                ...
                <input type="text" value="<?php echo htmlspecialchars($this->_var['img']['img_desc']); ?>" size="15" name="old_img_desc[<?php echo $this->_var['img']['img_id']; ?>]" /><br />
                颜色：<select class='old_img_color' gid="<?php echo $this->_var['img']['img_id']; ?>" name='old_img_color[<?php echo $this->_var['img']['img_id']; ?>]'><option value="0">-请选择-</option>
                        <?php $_from = $this->_var['color_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'v');if (count($_from)):
    foreach ($_from AS $this->_var['v']):
?>
                            <option value="<?php echo $this->_var['v']['name']; ?>" <?php if ($this->_var['v']['name'] == $this->_var['img']['img_color']): ?>selected="selected"<?php endif; ?>><?php echo $this->_var['v']['name']; ?></option>
                        <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
                      </select>
              </div>
              <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
          ...
          <tr>
            <td>
              ...
              颜色:<select name="img_color[]"><option value="0">-请选择-</option>
                <?php $_from = $this->_var['color_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'v');if (count($_from)):
    foreach ($_from AS $this->_var['v']):
?>
                    <option value="<?php echo $this->_var['v']['name']; ?>"><?php echo $this->_var['v']['name']; ?></option>
                <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
              </select>
...
$(function(){
    //img color
    $(".old_img_color").live('change',function(){
        var color = $(this).val();
        var id = $(this).attr('gid');
        $.post('goods.php?act=img_color',{'color': color, 'id': id},function(data){
            alert(data.msg);
        },'json');
    });
    //------------------------------------------------------------------------------------------------------
    ...
    var colorsObj = {};
    var sizesObj = {};
    $('.goods_attr_color').live('click',function(){
        var color = $(this).val();
        color = color.replace(/\n/,'');
        var extid = $(this).attr('extid');
        if('checked'==$(this).attr('checked')) {
            colors.push(color);
            colorsObj[extid] = color;
            goodsAttrExtColor(extid, color);
        } else {
            var gid = parseInt($('#goodsid').text());
            $.ajax({url: 'goods.php',
                    type: 'POST',
                    data: {'act': 'delGoodsAttrExt', 'gid': gid, 'color': color},
                    dataType: 'JSON',
                    async: false,
                    success: function(data){
                        if (200 == data.status && data.responseText) {
                            colors.remove(color);
                            delete(colorsObj[extid]);
                            $('#goods_attr_ext_table tr[cextid='+extid+']').remove();
                        }
                    },
                    error: function(data){
                        if (200 == data.status && data.responseText) {
    ...
    $('.goods_attr_size').live('click',function(){
        var size = $(this).val();
        size = size.replace(/\n/,'');
        var extid = $(this).attr('extid');
        if('checked'==$(this).attr('checked')) {
            sizes.push(size);
            sizesObj[extid] = size;
            goodsAttrExtSize(extid,size);
        } else {
            var gid = parseInt($('#goodsid').text());
            $.ajax({url: 'goods.php',
                    type: 'POST',
                    data: {'act': 'delGoodsAttrExt', 'gid': gid, 'size': size},
                    dataType: 'JSON',
                    async: false,
                    success: function(data){
                        if (200 == data.status && data.responseText) {}
            ...
            sizes.remove(size);
            delete(sizesObj[extid]);
            for (i in colorsObj) {
                if ('string' == typeof colorsObj[i]) {
                    var Obj = $('#goods_attr_ext_table [cextid='+i+']');
                    var rowspans = parseInt(Obj.eq(0).children("td").eq(0).attr('rowspan'));
                    rowspans -= 1;
                    Obj.eq(0).children("td").eq(0).attr('rowspan', rowspans);
                }
            }
            
            var Obj = $('#goods_attr_ext_table tr[sextid='+extid+']');
            for (var i=0, iMax = Obj.length; i < iMax; i++) {
                var tr0 = $('#goods_attr_ext_table tr[sextid='+extid+']').eq(i).children("td").eq(0).attr('rowspan');
                if(tr0) {
                    var td0 = $('#goods_attr_ext_table tr[sextid='+extid+']').eq(i).children("td").eq(0).text();
                    $('#goods_attr_ext_table tr[sextid='+extid+']').eq(i).next().prepend("<td rowspan='"+tr0+"'>"+td0+"</td>");
                }
            }
            $('#goods_attr_ext_table tr[sextid='+extid+']').remove();
        }
    });
    function goodsAttrExtColor(extid,color)
    {
        if(0 == sizes.length || 0 == colors.length) return false;
        var str = '';
        var j = 0;
        for (var i in sizesObj) {
            if ('string' == typeof sizesObj[i]) {
                    var goodsAttrExtId = addGoodsAttrExt(color, sizesObj[i]);
                    if (0 == j){
                        if (goodsAttrExtId) {
                            str += "<tr cextid='"+extid+"' sextid='"+i+"'><td rowspan='"+sizes.length+"'>"+color+"</td><td>"+sizesObj[i]+"</td>"+
                                    "<td><input type='text' goodsAttrId="+goodsAttrExtId+" /></td>"+
                                    "<td><input type='text' goodsAttrId="+goodsAttrExtId+" /></td>"+
                                    "<td><input type='text' goodsAttrId="+goodsAttrExtId+" /></td></tr>";
                        }
                    }else{
                        if (goodsAttrExtId) { 
                            str += "<tr cextid='"+extid+"' sextid='"+i+"'><td>"+sizesObj[i]+"</td>"+
                                    "<td><input type='text' goodsAttrId="+goodsAttrExtId+" /></td>"+
                                    "<td><input type='text' goodsAttrId="+goodsAttrExtId+" /></td>"+
                                    "<td><input type='text' goodsAttrId="+goodsAttrExtId+" /></td></tr>";
                        }
        ...
        $('#goods_attr_ext_table').append(str);
    }
    function goodsAttrExtSize(extid, size)
    {
        if(0 == sizes.length || 0 == colors.length) return false;
        var str = '';
        for (var i in colorsObj) {
            if ('string' == typeof colorsObj[i]) {
                var Obj = $("#goods_attr_ext_table tr[cextid="+i+"]");
                if(0 < Obj.length) {
                    var goodsAttrId = addGoodsAttrExt(colorsObj[i], size);
                    if (goodsAttrId) {
                        var str = "<tr cextid='"+i+"' sextid='"+extid+"'><td>"+size+"</td>"+
                                    "<td><input type='text' goodsAttrId="+goodsAttrId+" /></td>"+
                                    "<td><input type='text' goodsAttrId="+goodsAttrId+" /></td>"+
                                    "<td><input type='text' goodsAttrId="+goodsAttrId+" /></td></tr>";
                        var rowspans = parseInt(Obj.eq(0).children("td").eq(0).attr('rowspan'));
                        rowspans += 1;
                        Obj.eq(0).children("td").eq(0).attr('rowspan',rowspans);
                        Obj.last().after(str);
                    ...
                    var goodsAttrId = addGoodsAttrExt(colorsObj[i],size);
                    if (goodsAttrId)
                        $("#goods_attr_ext_table").append("<tr cextid='"+i+"' sextid='"+extid+"'><td rowspan='1'>"+colorsObj[i]+"</td>"+
                                                            "<td>"+size+"</td><td><input type='text' goodsAttrId="+goodsAttrId+" /></td>"+
                                                            "<td><input type='text' goodsAttrId="+goodsAttrId+" /></td>"+
                                                            "<td><input type='text' goodsAttrId="+goodsAttrId+" /></td></tr>");
                }
            }
...
function addGoodsAttrExt(color, size)
{
    var gid = parseInt($('#goodsid').text());
    var goodsAttrExtId;
    $.ajax({type: 'POST',
            url: 'goods.php',
            async: false,
            data: {'gid': gid,'color': color,'size': size, 'act': 'addGoodsAttrExt'},
            dataType: 'json',
            //ajax really success, maybe disturbed
            success: function(data){
                if (200 == data.status && data.responseText)
                    goodsAttrExtId = data.responseText;
            },
            error: function(data){
                if (200 == data.status && data.responseText)
                    goodsAttrExtId = data.responseText;
