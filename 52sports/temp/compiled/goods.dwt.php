...
?>
      ...
      <?php if ($this->_var['goods_color'] != false): ?>
      <li class="clearfix" id="goods_color">
        <?php $_from = $this->_var['goods_color']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'v');if (count($_from)):
    foreach ($_from AS $this->_var['v']):
?>
            <a href='javascript:;' onclick="javascript:changeColor(<?php echo $this->_var['goods']['goods_id']; ?>,'<?php echo $this->_var['v']['img_color']; ?>');"><?php echo $this->_var['v']['img_color']; ?></a>
        <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
      </li>
      <?php endif; ?>
