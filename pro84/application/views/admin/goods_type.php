...
function saveGoodsAttr()
{
    ...
    $.post('/admingoods/addtype', {'pid': pid, 'name': name}, function(data){
        alert(data.msg, '温馨提示');
        if (0 == data.err) {
...
function delGoodsAttr(id)
{
    ...
    if (id) {
        $.post('/admingoods/deltype', {'id': id}, function(data){
            alert(data.msg, '温馨提示');
...
$(function(){
    $('.goodsattrname').live('change',function(){
        ...
        if (id && name) {
            $.post('/admingoods/updatetype', {'id': id, 'name': name}, function(data){
                alert(data.msg);
