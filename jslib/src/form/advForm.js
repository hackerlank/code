/**
 * @fileOverview app jslib 高级表单验证类
		...
		$app.addEvent(_formobj,'submit',function(e){
			...
			for(o in _elements){
				...
				if(datatype != null && typeof(datatype) != 'undefined'){
					...
					if (flag == true) {
						msgs = obj.getAttribute('msg');
						$app.G(errid).innerHTML = '<span style="color:red">'+msgs+'</span>';
						$app.G(errid).style.display = 'block';
						return;
					}
				}
			}
			...
			var subtype = submitType.toLowerCase();
		
    		if(subtype == 'get'){
    		    //载入中提示
    		    var errObj = document.getElementById(errid);
    		    if(errObj){
    		        errObj.innerHTML = '<span style="color:red">载入中，请稍后...</span>';
    		        errObj.style.display = 'block';
    		    }
				loader('ajax/ajax',function(){
					$app.ajax.getJSON(url+'?dt='+dt+'&'+datastr,function(e){
						callback(e);
					});
				});
    		}
				...
				loader('ajax/ajax',function(){
					$app.ajax.post(url+'?dt='+dt,data,function(e){
						callback(e);
					},'json');
				});
    		}
    		else if(subtype == 'jsonp'){
    		    //载入中提示
    		    var errObj = document.getElementById(errid);
    		    if(errObj){
    		        errObj.innerHTML = '<span style="color:red">载入中，请稍后...</span>';
				...
				loader('ajax/ajax',function(){
					$app.ajax.getJSONP(url+'?'+datastr,function(e){
						callback(e);
					});
				});
    		}
