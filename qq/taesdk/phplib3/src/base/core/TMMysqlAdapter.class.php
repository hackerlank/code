...
class TMMysqlAdapter
{
    ...
    public function openConnection($dbhost, $dbuser, $dbpasswd, $database, $port = 3306, $autoCommit=TRUE,$code='UTF8')
    {
        ...
        if (! $this->connection)
        {
            throw new TMMysqlConnectionException("Can't connect the database ".$database." in ".$dbuser."@".$dbhost."used password ".$dbpasswd." ".mysqli_connect_error($this->connection));
        }
    ...
    public function query($sql, $resultType = MYSQLI_BOTH)
    {
        ...
        //TMHook::call("before_sql_execute", array($sql));
